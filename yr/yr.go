...
import (
	...
	"github.com/Orhanyil/funtemps/conv"
)

func ConvertTemperature() {
	overwriteFile := checkFileExists()
	if !overwriteFile {
		fmt.Println("Tilbake til hovedsiden")
		return
	}
...
func checkFileExists() bool {
	if _, err := os.Stat("kjevik-temp-fahr-20220318-20230318.csv"); err == nil {
		fmt.Print("Det er en fil med samme navn. Vil du gjenoppbygge filen? (j/n): ")
...
func ProcessLine(line string) string {
	...
	if line[0:7] == "Data er" {
		return "Data er basert på gyldig data (per 18.03.2023) (CC BY 4.0) fra Meteorologisk institutt (MET);endringen er gjort av Orhan"
	} else {
...
func AverageTemperature() {
	...
	fmt.Println("Vennligts Velg (celsius/fahr):")
	var unit string
	fmt.Scan(&unit)
